...
description: "Converges existing source-code edits in guided / vibe coding, removes superseded-approach residue, and produces reviewer-friendly CRs. MUST be used for focused edits to existing program files (.ts/.tsx/.js/.jsx/.py/.go/.java/.vue/.css/.scss/.json/.yaml, etc.), including small additions, deletions, hooks, optimizations, config updates, and logic changes. Signals: minimal change, diff too large, do not over-modify, converge changes, switch A to B, previous change was wrong, revert changes, stale code, clean up residue, unclear CR, or too many comments. Load it without those phrases whenever existing program files receive focused edits, a diff is bloated, an old approach remains, or review readability declines. Also use for narrow updates to existing README developer commands or environment setup, but not general docs, changelogs, or wikis. Do not use for greenfield projects or modules, brand-new files, or architecture-scale refactors spanning dozens of files."
---
